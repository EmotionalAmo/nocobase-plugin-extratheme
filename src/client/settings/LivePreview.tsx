...
export const LivePreview: React.FC<{ scope: 'app' | 'login'; app: AppConfig; login: LoginConfig }> = ({
  ...
  const t = useT();
  if (scope === 'app') {
    // Content cards stay OPAQUE — card transparency is delegated to the native theme editor.
    // The page background still shows through the layout gaps around the cards.
    const cardStyle: React.CSSProperties = { flex: 1, borderRadius: 10, padding: 12, background: '#fff', border: '1px solid rgba(0,0,0,0.06)', color: '#1f2733', fontSize: 12 };
